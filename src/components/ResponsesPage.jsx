...
export default function ResponsesPage() {
  ...
  const exportSinglePDF = async (response) => {
    ...
    const [header1Base64, header2Base64, footerBase64] =
      await Promise.all([getBase64FromUrl(header1), getBase64FromUrl(header2), getBase64FromUrl(footer)]);
  ...
  const exportCommentsByBlockPDF = async (response) => {
    if (!response) return;
    ...
    doc.setFont("helvetica", "bold");
    doc.setFontSize(16);
    doc.text("COMENTÁRIOS POR BLOCO", pageW / 2, y, { align: "center" });

    y += 30;

    Object.entries(response.blocos || {}).forEach(([_, bloco]) => {
      if (!bloco.comentario?.resposta) return;
    ...
    doc.save("comentarios_por_bloco.pdf");
  };
        ...
        <div className="fixed inset-0 bg-black/40 flex justify-center items-center p-4">
          <div className="bg-white rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto p-8 relative">
            ...
            <div className="flex justify-between mt-6">
              ...
              <button
                onClick={() => exportCommentsByBlockPDF(selectedResponse)}
                className="px-6 py-3 bg-blue-600 text-white rounded"
              >
                Exportar Comentários por Bloco
              </button>
            </div>
          </div>
        </div>
